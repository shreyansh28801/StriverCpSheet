...
void solve()
{
    ...
    {
        n++;
        v.pub(INT_MAX); //inserting values such that it should npt affect the original array
    }

    //building of segment otree
    ...
    function<ll(ll, ll, ll, ll, ll)> query = [&](ll node, ll lx, ll rx, ll l, ll r) {
        if(l>rx || lx>r)    return (ll)INT_MAX;
        if(lx>=l && r>=rx)    return segTree[node];
